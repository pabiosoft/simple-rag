...
type AppConfig = {
  ...
  answerMinSentences: number;
  answerMaxSentences: number;
  assistantMode: 'strict' | 'hybrid';
  noContextBehavior: 'guide' | 'general';
...
const baseConfigPath = path.resolve('config/settings/app.yaml');
const envConfigPath = path.resolve(`config/settings/app.${process.env.NODE_ENV || 'development'}.yaml`);
...
const appConfig: AppConfig = {
  ...
  answerMinSentences: Number(process.env.ANSWER_MIN_SENTENCES || rawConfig.answerMinSentences || 4),
  answerMaxSentences: Number(process.env.ANSWER_MAX_SENTENCES || rawConfig.answerMaxSentences || 8),
  assistantMode: ((process.env.ASSISTANT_MODE || rawConfig.assistantMode || 'strict') as 'strict' | 'hybrid'),
  noContextBehavior: ((process.env.NO_CONTEXT_BEHAVIOR || rawConfig.noContextBehavior || 'guide') as 'guide' | 'general'),
  promptProfile: process.env.PROMPT_PROFILE || rawConfig.promptProfile || '',
  country: process.env.APP_COUNTRY || rawConfig.country || '',
